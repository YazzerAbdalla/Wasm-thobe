...
|---|---|
| `AuthModule` | Admin login, JWT access + refresh token flow |
| `UsersModule` | User entity, profile (primarily for admin management) |
|| `CustomizationModule` | Colors, fabrics, accessories, recommendation engine |
| `OrdersModule` | Create order (supports guest users), admin status update |
